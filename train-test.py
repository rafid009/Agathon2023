...
def evaluate_imputation(df, mean, std, models, mse_folder, test_idx=-1, trials=30, given_features=['precipitation']):
    ...
    if trials == 1:
        fp = open(f"{mse_folder}/all-sample-results_{test_idx}.json", "w")
        json.dump(results, fp=fp, indent=4, cls=NumpyArrayEncoder)
        fp.close()
    else:
        out_file = open(f"{mse_folder}/model_mse_{test_idx}.json", "w")
...
def run_train_test (df, config_dict, mse_folder, model_folder, n_test=5, seed=0):
    ...
    for test_idx in test_indices:
        ...
        train(
            ...
            filename=f"model_diffsaits_{test_idx}.pth"
        )
            
        evaluate_imputation(df, mean, std, models, mse_folder, test_idx=test_idx, given_features=['precipitation'])
        evaluate_imputation(df, mean, std, models, mse_folder, test_idx=test_idx, given_features=['precipitation'], trials=1)
